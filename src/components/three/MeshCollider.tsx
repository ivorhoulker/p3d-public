...
export default function MeshCollider(props: any) {
  ...
  if (props.position instanceof Array)
    position.set(...(props.position as [number, number, number]));
  if (props.position instanceof Vector3) position.copy(props.position);
  if (props.rotation instanceof Array)
    rotation.set(...(props.rotation as [number, number, number]));
  if (props.rotation instanceof Euler) rotation.copy(props.rotation);
  if (props.scale instanceof Array)
    scale.set(...(props.scale as [number, number, number]));
