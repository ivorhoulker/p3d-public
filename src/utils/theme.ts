export const PALETTE = {
  pastelPurple: 0xa58fff,
  pastelOrange: 0xffb152,
}

export const WALL_COLOR = 'white'
export const GROUND_COLOR = 0xff8e63
export const FOG_COLOR = 'black'
export const SKY_TOP_COLOR = 0x2d0045// 0x1f0073 // 0x2d0045
export const SKY_BOTTOM_COLOR = 0x82a5ff // 0xc97dff // 0x0077ff
export const DEFAULT_CAMERA_Y = 5
export const DEFAULT_CAMERA_Z = -10
export const CAR_BASE_HEIGHT = 0.7
export const CAR_SPEED = 10
export const CAR_ROTATION_SPEED = 100